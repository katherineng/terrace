...
public class SelectionRecorder{
	...
	Optional<Integer> exitSelectionMode() {
		...
		for (int i = 0; i < hits; i++) {
			...
			if (m_selectionBuffer.get(hit+1) < minDepth) {
				index = Optional.of(m_selectionBuffer.get(hit + 3));
		...
		if (index.isPresent()) {
			...
			float[] hitm = new float[] {0f, 0f,0f};
			glu.gluUnProject(
					(float)m_mouseX,
					(float)m_mouseY,
					(float)minDepth / (float)0xFFFFFFFF,
					modelBuffer,
	        		projectBuffer,
	        		IntBuffer.wrap(viewport),
	        		FloatBuffer.wrap(hitm)
			);
		}
		return index;
	}
