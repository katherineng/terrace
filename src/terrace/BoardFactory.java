...
public class BoardFactory {
	...
	private static void setUpDefault2P(Player p1, Player p2, Board board) {
		...
		for (int i = 0; i < dim; i++) {
			if (i == 0) {
				Piece tPiece = new TPiece(smallCorner, p1, largeCorner);
				...
				Piece tPiece = new TPiece(largeCorner, p2, smallCorner);
	...
	private static void setUpDefault4P(Player p1, Player p2, Player p3, Player p4, Board board) {
		...
		for (int i = 1; i < dim - 1; i++) {
			if (i == 1) {
				Piece t1Piece = new TPiece(new Posn(1, 0), p1, largeCorner);
				Piece t2Piece = new TPiece(new Posn(0, 1), p2, largeCorner);
				...
				Piece t3Piece = new TPiece(new Posn(i, dim - 1), p3, smallCorner);
				Piece t4Piece = new TPiece(new Posn(dim - 1, i), p4, smallCorner);
				p1.addPiece(p1Piece);
				p2.addPiece(p2Piece);
				p3.addPiece(t3Piece);
				p4.addPiece(t4Piece);
